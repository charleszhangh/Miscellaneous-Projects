...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
         ...
         */
        if (!m.isZero()) {//ensures it is as least one
            n.copyFrom(n.divide(m));//n becomes the modulus
            reduceToGCD(m, n);
            n.transferFrom(m);
        }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber replacement = new NaturalNumber2(n);
        NaturalNumber result = replacement.divide(two);

        if (result.compareTo(zero) != 0) {//if there is no remainder
            trueFalse = false;
        }
        return trueFalse;
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {//result is one if power is zero
            n.transferFrom(one);
        } else {
            temp.decrement();
            for (NaturalNumber count = new NaturalNumber2(); count
                    .compareTo(temp) < 0; count.increment()) {//puts n to the power of p
                n.multiply(original);
            }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber secondw = new NaturalNumber2(w);
        n.decrement();
        NaturalNumber nMinus = new NaturalNumber2(n);
        n.increment();
        powerMod(w, two, n);//runs the powermod method
        powerMod(secondw, nMinus, n);
        if (w.equals(one) || !secondw.equals(one)) {//two conditions
            trueFalse = true;
        }
        return trueFalse;
    }
    ...
    public static boolean isPrime1(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
             ...
             */
            isPrime = !isWitnessToCompositeness(new NaturalNumber2(2), n);//if it's not composite, it's prime
        }
        return isPrime;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        assert n.compareTo(new NaturalNumber2(1)) > 0 : "Violation of: n > 1";
        NaturalNumber[] witnessc = new NaturalNumber2[50];
        boolean trueFalse = false;
        for (int i = 0; i < 50; i++) {//runs fifty times
            n.decrement();
            witnessc[i] = randomNumber(n);
            n.increment();
            if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
                /*
                 * 2 and 3 are primes
                 ...
                 * composite (which works surprisingly well :-)
                 */
                trueFalse = !isWitnessToCompositeness(
                        new NaturalNumber2(witnessc[i]), n);
            }
        }
        /*
