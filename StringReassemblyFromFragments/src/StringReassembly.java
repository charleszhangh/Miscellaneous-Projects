...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        String combined = "";
        combined = str1 + str2.substring(overlap, str2.length());
        return combined;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        Set<String> repeats = new Set1L<>(); //checks for repeats in substrings
        for (String s : strSet) {
            for (String r : strSet) {
                if (s.indexOf(r) != -1 && s.length() != r.length()) {
                    repeats.add(r);
                }
            }
        }
        strSet.remove(repeats);
        Set<String> takeOut = new Set1L<>(); //set for items that need to be taken out
        Set<String> putIn = new Set1L<>(); //items that need to be added in
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        while (!input.atEOS()) {
            int sep = input.nextLine().indexOf("~");
            out.println(text.substring(0, sep));
        }
    }
